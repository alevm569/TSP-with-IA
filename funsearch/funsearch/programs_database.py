...
class ProgramsDatabase:
    ...
    def get_prompt(self) -> Prompt:
        ...
        if getattr(self, "_allow_final_prompt", False):
            logging.info("Enviando último prompt antes de detener FunSearch.")
            self._allow_final_prompt = False  # Desactivamos el flag después del último prompt
            return self._get_best_final_prompt()

        if getattr(self, "_stop_search", False):
            logging.info("Se alcanzó el mejor score en todas las islas. No se generarán más prompts.")
            return None  # Indicar a FunSearch que ya no hay más prompts

            # Si la búsqueda sigue, seleccionar aleatoriamente una isla
        island_id = np.random.randint(len(self._islands))
        code, version_generated = self._islands[island_id].get_prompt()
        return Prompt(code, version_generated, island_id)

    def _get_best_final_prompt(self) -> Prompt:
        """Genera un último prompt con el mejor programa encontrado."""
        best_island_index = np.argmax(self._best_score_per_island)
        best_program = self._best_program_per_island[best_island_index]

        if best_program is not None:
            return Prompt(str(best_program), version_generated=0, island_id=best_island_index)
    ...
    def _register_program_in_island(
        ...
        """Registers `program` in the specified island and stops if reach the best score."""
        ...
        if self._stop_search:
            return  # Si ya se decidió detener, no registrar más programas
        ...
        if score > self._best_score_per_island[island_id]:
            ...
            self._best_scores_per_test_per_island[island_id] = scores_per_test
            self._best_score_per_island[island_id] = score
            logging.info("Nuevo mejor score en isla %d: %s", island_id, score)

        # 🔹 Verificamos si todas las islas han alcanzado su mejor score
        best_global_score = max(self._best_score_per_island)
        if all(score >= best_global_score for score in self._best_score_per_island):
            logging.info("Todas las islas han alcanzado su mejor score. Deteniendo FunSearch.")
            self._allow_final_prompt = True  # Activamos el flag para detener la búsqueda
