...
def find_best_route(distances: np.ndarray) -> tuple[int, ...]:
    ...
    Combines the nearest neighbor and cheapest insertion heuristics with a local search approach.
    """
    ...
    # Local search with cheapest insertion heuristic
    for i in range(len(route)):
        for j in range(i + 1, len(route)):
            new_route = route[:i] + route[j:j+1] + route[i+1:j] + route[j+1:]
